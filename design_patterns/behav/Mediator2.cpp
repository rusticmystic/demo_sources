 ...
 * proliferating interconnections tend to reduce it again. You can avoid this problem by capsulating 
 * the interconnections (i.e. the collective behavior) in a separate "mediator" object. A mediator is 
 * responsible for controlling and coordinating the interactions of a group of objects. 
 * In this example, the dialog box object is functioning as the mediator. Child widgets of the dialog box do not know,
 * or care, who their siblings are. Whenever a simulated user interaction occurs in a child widget Widget::changed(),
 ...
 * (it serves as the hub of communication). The user may choose to "interact" with a 
 * simulated: filter edit field, directories list, files list, or selection edit field
*/
