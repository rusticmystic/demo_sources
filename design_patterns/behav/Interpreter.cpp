 ...
 * This is related to grammar and languages
 * 
 * Map a domain to a language, the language to a grammar, and the grammar to a hierarchical object-oriented design
 * 
 * An abstract base class specifies the method interpret(). Each concrete subclass implements interpret()
 * by accepting (as an argument) the current state of the language stream, and adding its contribution to the
 *  problem solving process
 * 
 * Interpreter suggests modeling the domain with a recursive grammar. 
 * Each rule in the grammar is either a 'composite' (a rule that references other rules) 
 * or a terminal (a leaf node in a tree structure). Interpreter relies on the recursive traversal
 * of the Composite pattern to interpret the 'sentences' it is asked to process
 * 
 * Interpreter can use State to define parsing contexts.
 * 
 * The abstract syntax tree of Interpreter is a Composite (therefore Iterator and Visitor
 * are also applicable). Terminal symbols within Interpreter's abstract syntax tree can be
 * shared with Flyweight. The pattern doesn't address parsing. When the grammar is very complex, other 
 * 
 * */
  ...
  * (remaining unparsed string and cumulative parsed value) and contributes its share to the processing. Sub-interpreters 
  * simply define the Template Methods declared in the base class RNInterpreter.
